...
func TestGetConfiguration(t *testing.T) {
	...
	c := Get()
	if c.Dumbo1.NetworkSize != 10 {
		t.Fatalf("expected N is %d, but got %d", 10, c.Dumbo1.NetworkSize)
	}
	if c.Dumbo1.Byzantine != 3 {
		t.Fatalf("expected F is %d, but got %d", 3, c.Dumbo1.Byzantine)
	}
