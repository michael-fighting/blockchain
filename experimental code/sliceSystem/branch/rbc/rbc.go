...
func New(
	...
	//enc, err := rscode.NewMockEncoder(numDataShards, numParityShards)
	...
	rbc := &RBC{
		...
		log: cleisthenes.NewLoggerWithHead("RBC"),
	}
...
func (rbc *RBC) HandleInput(data []byte) error {
	...
	rbc.log.Debug().Msgf("shard time:%v", end.Sub(start))
...
func (rbc *RBC) HandleMessage(sender cleisthenes.Member, msg *pb.Message_Rbc) error {
	...
	rbc.log.Debug().Msgf("process message time:%v, msgType:%v sender:%v", end.Sub(start), msg.Rbc.Type, sender.Address.String())
...
func (rbc *RBC) handleEchoRequest(sender cleisthenes.Member, req *EchoRequest) error {
	...
	t3 := time.Now()
	rbc.log.Debug().Msgf("t3 time:%v", t3.Sub(t2))
	//正常情况不会进入下面这个循环，会走358行解码RBC成功
	//如果收到了2f+1个ready，但是没有收到足够的echo，会在这里等待收到足够的（n-f）echo信息后对tx进行还原（纠删码）
	rbc.log.Debug().Msgf("rbc:%p recv echo:%v, ready cnt:%d, echo cnt:%d sender:%s", rbc, req.RootHash, rbc.countReadys(req.RootHash), rbc.countEchos(req.RootHash), sender.Address.String())
	...
	rbc.log.Debug().Msgf("t4 time:%v", end.Sub(t3))
...
func (rbc *RBC) handleReadyRequest(sender cleisthenes.Member, req *ReadyRequest) error {
	...
	rbc.log.Debug().Msgf("rbc:%p recv ready:%v, ready cnt:%d, echo cnt:%d outputThreshold:%d echoThreshold:%d sender:%s",
		rbc, req.RootHash, rbc.countReadys(req.RootHash), rbc.countEchos(req.RootHash), rbc.outputThreshold(), rbc.echoThreshold(), sender.Address.String())
...
func (rbc *RBC) tryDecodeValue(rootHash []byte) ([]byte, error) {
	...
	if len(reqs) < rbc.numDataShards {
		rbc.log.Error().Msgf("not enough shards - minimum : %d, got : %d ", rbc.numDataShards, len(reqs))
	...
	if err := rbc.enc.Reconstruct(shards); err != nil {
		rbc.log.Error().Msgf("reconstruct failed, err:%s", err.Error())
		return nil, err
	}

	end := time.Now()
	rbc.log.Debug().Msgf("reconstruct time:%v", end.Sub(start))
	...
	var res []byte
	res = append(res, value[:rbc.contentLength.value()]...)
