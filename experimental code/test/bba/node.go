...
import (
	bba2 "github.com/DE-labtory/cleisthenes/sliceSystem/branch/bba"
	"time"
...
func New(typ NodeType, n, f int, coinGenerator cleisthenes.CoinGenerator, addr cleisthenes.Address) (*Node, error) {
	...
	bba := bba2.New(n, f, 0, *member, cleisthenes.Member{}, connPool, binChan)
	return &Node{
		typ:       typ,
		addr:      addr,
		bba:       bba,
...
func (n *Node) Propose(bin cleisthenes.Binary) error {
	bvalRequest := &bba2.BvalRequest{Value: bin}
